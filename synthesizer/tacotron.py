...
class BatchNormConv(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x)
        x = F.relu(x) if self.relu else x
        return self.bnorm(x)
    ...
    def forward(self, x, m, speaker_embedding):
        ...
        for t in range(0, steps, self.r):
            ...
            mel_frames, scores, hidden_states, cell_states, context_vec, stop_tokens = self.decoder(
                encoder_seq, encoder_seq_proj, prenet_in, hidden_states, cell_states, context_vec, t, x)
...
class WaveNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, residual_channels=512, gate_channels=512, skip_channels=256, aux_channels=80, num_blocks=3, num_layers=10, kernel_size=3, dropout=0.05):
        super().__init__()
        ...
        for b in range(num_blocks):
            for i in range(num_layers):
                ...
                self.residual_layers.append(nn.Conv1d(residual_channels, residual_channels, kernel_size, padding=padding, dilation=dilation))
                self.gate_layers.append(nn.Conv1d(residual_channels, gate_channels, kernel_size, padding=padding, dilation=dilation))
                self.skip_layers.append(nn.Conv1d(gate_channels, skip_channels, kernel_size=1))
                self.aux_layers.append(nn.Conv1d(aux_channels, gate_channels, kernel_size=1))
    ...
    def forward(self, x, aux):
        ...
        for i in range(self.num_blocks * self.num_layers):
            residual = self.residual_layers[i](h)
            gate = torch.sigmoid(self.gate_layers[i](h) + self.aux_layers[i](aux))
            h = residual * gate
            skip += self.skip_layers[i](h)
        out = self.output_conv(skip)
        return out

class Tacotron2WaveNet(nn.Module):
    def __init__(self, tacotron_config, wavenet_config):
        super().__init__()
        self.tacotron2 = Tacotron2(**tacotron_config)
        self.wavenet = WaveNet(**wavenet_config)

    def forward(self, text, mel_target, speaker_embedding=None):
        mel_output, linear_output, attn_scores, stop_tokens = self.tacotron2(text, mel_target, speaker_embedding)
        waveform = self.wavenet(mel_output)
        return waveform, mel_output, linear_output, attn_scores, stop_tokens

    def load_state_dict(self, state_dict, strict=True):
        tacotron_state_dict = {k.replace("tacotron2.", ""): v for k, v in state_dict.items() if k.startswith("tacotron2.")}
        wavenet_state_dict = {k.replace("wavenet.", ""): v for k, v in state_dict.items() if k.startswith("wavenet.")}

        self.tacotron2.load_state_dict(tacotron_state_dict, strict)
        self.wavenet.load_state_dict(wavenet_state_dict, strict)
